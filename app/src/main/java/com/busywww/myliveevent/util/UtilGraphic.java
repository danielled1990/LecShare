...
//                            | ((g >> 2) & 0xff00) | ((b >> 10) & 0xff) | 0xff000000;
    ...
    public static byte [] GetNV21FromBitmap(Bitmap bitmap, int jpegCompress, int scaleWidth, int scaleHeight) {
        byte [] yuv = null;
        Bitmap compressed = null;
        try {
            if (jpegCompress < 95) {
                //compressed = GetBitmapCompressed(bitmap, jpegCompress);
            ...
            if (inputWidth != scaleWidth || inputHeight != scaleHeight) {
                compressed = GetScaledBitmap(compressed, scaleWidth, scaleHeight);
                inputWidth = compressed.getWidth();
                inputHeight = compressed.getHeight();
                argb = new int[inputWidth * inputHeight];
//                compressed.getPixels(argb, 0, scaleWidth, 0, 0, scaleWidth, scaleHeight);
//                yuv = new byte[scaleWidth*scaleHeight*3/2];
//                EncodeBitmapToYUV420SP(yuv, argb, scaleWidth, scaleHeight);
            } else {
                inputWidth = compressed.getWidth();
                inputHeight = compressed.getHeight();
                argb = new int[inputWidth * inputHeight];
            }
            compressed.getPixels(argb, 0, inputWidth, 0, 0, inputWidth, inputHeight);
            yuv = new byte[inputWidth*inputHeight*3/2];
            EncodeBitmapToYUV420SP(yuv, argb, inputWidth, inputHeight);
    ...
    public static Bitmap GetBitmapCompressed(Bitmap bitmap, int jpegCompress) {
        Bitmap compressed = null;
        try {
            ByteArrayOutputStream bos = new ByteArrayOutputStream();
            bitmap.compress(Bitmap.CompressFormat.JPEG, jpegCompress, bos);
            byte[] bytes = bos.toByteArray();
            bos.close();

            compressed = BitmapFactory.decodeByteArray(bytes, 0, bytes.length);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return compressed;
    ...
    public static void EncodeBitmapToYUV420SP(byte[] yuv420sp, int[] argb, int width, int height) {
        final int frameSize = width * height;
